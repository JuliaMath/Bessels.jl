...
function airyai_power_series(z::Complex{T}) where T
    z2 = z * z
    z3 = z2 * z
    p = horner_simd(z3, pack_AIRYAI_POW_COEF)
...
function airybi_power_series(z::Complex{T}) where T
    z2 = z * z
    z3 = z2 * z
    p = horner_simd(z3, pack_AIRYBI_POW_COEF)
    bi = muladd(p[2], z, p[1])
...
@inline function compute_airy_asy_coef(z, xsqrx)
    ...
    p = horner_simd(invx3, pack_AIRY_ASYM_COEF)

    pvec1 = ComplexVec((p[1], p[3]))
    pvec2 = ComplexVec((p[2], p[4]))

    zvec = ComplexVec{2, Float64}((xsqrx.re, xsqrx.re), (xsqrx.im, xsqrx.im))
    zvec = fmul(zvec, pvec2)
    a = fadd(pvec1, zvec)
    b = fsub(pvec1, zvec)
...
const pack_AIRYAI_POW_COEF = pack_poly((
...
const pack_AIRYBI_POW_COEF = pack_poly((
