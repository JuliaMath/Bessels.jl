...
function airyai(x::Float64)
    if x >= 2.06
        return exp(-2 * (sqrt(x)^3 / 3)) * airyaix_large_pos_arg(x)
    else
        if x <= -9.5
            return airyai_large_neg_arg(x)
        else
            if x >= 0.0
...
function airyaix(x::Float64)
    if x >= 2.06
        return airyaix_large_pos_arg(x)
    elseif x >= 0.0
        # taylor expansion at x = 1.5 
...
@inline function airyaix_large_pos_arg(x::Float64)
    if x > 3e10
        x3 = x * x * x
        p = evalpoly(1 / x3, (1.5707963267948966, -0.1636246173744684, 0.13124057851910487))
        return p / (π^(3/2) * sqrt(sqrt(x)))
    else
        xsqr = sqrt(x)
        zinv = 3 / (2 * x * xsqr)
        ...
        return inv_sqpi * p / (q * 2 * sqrt(xsqr))
    end
end

@inline function airyai_large_neg_arg(x::Float64)
    x3 = x * x * x
    ...
    c = exp(2 * sq_x3 / 3) # loses some precision here
    a = muladd(1 / sq_x3, -q, p) / c
    b = muladd(1 / sq_x3, q, p) * c 
    return real((b + im*a) / (π^(3/2) * complex(x)^(1/4)))
end
